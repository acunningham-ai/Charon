...
- ✅ 13 hooks (load-rules, save-on-mention with Haiku Stage 2, poisoning-scan, deny-destructive, validate-write-path, validate-memory-frontmatter, voice-anchor-ralph-loop, skill-usage-log, ssh-recovery, notification-toast, check-reauth-flag, check-todo-freshness; plus on-error, invoked by scheduled runners on failure)
