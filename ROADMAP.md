...
- ✅ **Write-path confinement for interactive commands** (2026-08-05, `v0.26.0`, **ships in shadow**). `validate-write-path.py` only engages for unattended `claude -p` runs, so an interactive command's write path was governed by prose in its own definition and nothing else. `/meeting-prep` made it concrete: it derives an output filename from a person's name that **originated in a calendar event** — a string an attacker can influence by sending an invite. Its sanitisation rule is genuinely protective *if applied*, but that is model adherence, not a control.
  ...
  **How it knows which command is running.** A PreToolUse payload names no command, so the obvious route is grepping the prompt for `/name` — a guess, and a guess that blocks a write is a false positive with teeth. Claude Code invokes a typed command *through the `Skill` tool*, whose input carries the name, so `skill-usage-log.py` records it and the gate reads it. That is an observation, not an inference. The record is per-session and TTL-bounded, so one session's scope cannot constrain another's writes and an abandoned command stops constraining the rest of the session.

  Verdict is **`ask`**, never `deny`: each target is something a person could legitimately mean to write, and what is being prevented is a path the *model* derived wrongly. The hook fails **OPEN**, the opposite of its unattended sibling — that one guards runs nobody is watching, where crashing beats an unbounded write; this one runs interactively, where a bug in the gate would block a human's legitimate work with no way around it.

  Guarded by **D31**, which fires real payloads through the hook as a subprocess rather than reading it, and asserts on the emitted verdict (exit codes are 0 in shadow, so the verdict is the only honest signal). Negative-tested twice: removing a protected glob fails it, and stripping every `write-scope:` fails it with "layer 2 is inert".

  **Coverage is partial and reported as such: 5 of 24 write-capable commands declare a scope.** The five were chosen because their output path is stated unambiguously; guessing the other nineteen would ship false positives into a confinement control, which is how confinement controls get switched off. D31 prints the ratio every run, so the gap stays visible rather than reading as complete.

  **Honest limits:** it ships in shadow (`observe` + `would_be: ask`) and enforces nothing until `SHADOW = False` after a review window — a brand-new confinement gate that blocks on day one turns every unanticipated write into a broken command. An undeclared command is unenforced, not permitted-by-default, and logs `no-scope-declared` so the gap is auditable. And writes the model makes long after a command finishes fall outside the TTL window, deliberately.
...
- ✅ **All three workflows were unlaunchable on Windows — fixed** (2026-08-04, `v0.25.0`). The `Workflow` tool refuses a script containing control characters, and **CR (U+000D) is one**. With no `.gitattributes`, working-tree line endings are set by each user's `core.autocrlf`, which defaults to `true` on Git for Windows — so a fresh clone rewrote every LF to CRLF and `/review`, `/deep-research` and `/devils-advocate` all failed to launch. Confirmed by cloning the public repo with `-c core.autocrlf=true`: **3 of 3** workflow scripts came down CRLF.
  ...
  Fixed in two halves, since either alone is insufficient: **`.gitattributes`** pins `eol=lf` (authoritative over `core.autocrlf`, with `*.bat`/`*.cmd` kept CRLF for cmd.exe's label/`goto` parsing), and **D30 `check_workflow_scripts_launchable`** fails on any control character in a workflow script, on a missing `eol=lf` pin, and on CR anywhere in the tracked text tree. Negative-tested both ways.
  ...
  **Honest note:** the first attempt at this release reintroduced the same defect in the same commit, putting CRLF into `CHANGELOG.md` via Python's `write_text` (which emits `os.linesep` on Windows), and the then-narrower D30 passed it. That is why D30 sweeps the whole tree rather than only the three files that had already broken.
...
- ✅ **Publishing is now a checked property, not a habit** (2026-08-04, `v0.24.1`–`v0.24.3`). Two independent failures made the public site an unreliable narrator, and both are now mechanical rather than remembered.
  ...
  **Verified end-to-end (2026-08-04):** a push touching `docs/` fired the trigger and completed the deploy at HEAD, and a push touching no site file correctly did not deploy. Both directions of the paths filter and the full publish path are exercised. D29 proves the *files* are right; only reading the live URL proves the *page* is right, so the release ritual still ends at the URL. And nothing yet gates deploy on the check suite — a push that skipped the local gates could still publish.
...
- ✅ 16 hooks (load-rules, save-on-mention with Haiku Stage 2, poisoning-scan, poisoning-scan-read, deny-destructive, validate-write-path, validate-interactive-write, validate-memory-frontmatter, phase-gate, cerberus/scan-config-edits, voice-anchor-ralph-loop, skill-usage-log, ssh-recovery, notification-toast, check-reauth-flag, check-todo-freshness; plus on-error, invoked by scheduled runners on failure)
