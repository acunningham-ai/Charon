...
The user wants a natural-language graph query. Use the `vault-query` skill.

The skill parses the user's question into one of four subcommands and invokes `python scripts/vault_query.py`:
...
Requires the optional networkx dep (`pip install -r requirements-graph.txt`) and a populated vault graph (run `python scripts/extract_entities.py` first). The skill detects when those preconditions aren't met and prints a clear pointer.
