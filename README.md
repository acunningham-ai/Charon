...
| **Path-conditioned rules** | 14 path-rules auto-inject doctrine when the path/keyword fires (board-reporting, ai-governance, secure-code, captures, quarterly-report, voice-content, skill-authoring, verdict-vocabulary, versioning, design, backlink-discipline, clean-room-port, thoroughness-over-false-efficiency, clean-signal-gate) | `.claude/rules/*.md` + `scripts/load-rules.py` (UserPromptSubmit hook) |
...
`test-scenarios/` ships with the harness — 16 LLM-behaviour scenarios + 28 automated deterministic checks. The same suite runs before any release and after any material change to rules / hooks / wizard. Pass-rate threshold is published; releases with a failing scenario must document it in known-limitations.
...
| **Path-conditioned rules** | 14 rules — auto-injected on path/keyword match: board-reporting, ai-governance, secure-code, captures, quarterly-report, voice-content, skill-authoring, verdict-vocabulary, versioning, design, backlink-discipline, clean-room-port, thoroughness-over-false-efficiency, clean-signal-gate |
| **Slash commands** | 55 commands across reporting + governance, security review, the research→compose pipeline (`/prometheus`, `/calliope`, `/forum-agenda`), the knowledge-graph build (`/graph-backfill`, `/vault-query`), retrieval + session (`/recall`, `/find-tensions`, `/grill`, `/handoff`), the governed self-* pair (`/harness-doctor`, `/harness-improve`), doc/web utilities (`/ingest`, `/webfetch`, `/docs`, `/skill-eval`), workflow, hygiene, and the 5-command Cerberus suite — full catalogue with fire conditions in [`CAPABILITIES.md`](CAPABILITIES.md) |
| **Hooks** | 13 hooks — load-rules (rule injection), save-on-mention (two-stage Haiku-classified), poisoning-scan (screens untrusted captured content), deny-destructive, validate-write-path, validate-memory-frontmatter, voice-anchor-ralph-loop, skill-usage-log, ssh-recovery, notification-toast, check-reauth-flag, check-todo-freshness, validate-interactive-write (interactive write confinement, ships in shadow) — plus `on-error.py`, invoked by scheduled runners on failure |
| **MCP servers** | 4 local stdio servers (3 on by default + 1 opt-in) — `vault-readonly` (keyword + semantic search, unit context, initiatives), `vault-ops` (patch_note, frontmatter_query, manage_tags), `vault-graph` (entity / relationship queries, networkx-backed, read-only), and **`calendar`** — a read-only Microsoft/Google calendar reader, **opt-in and off by default**: it needs your own OAuth client ID, stores no client secret, requests the narrowest read scope each provider offers, and fails closed if handed a broader one (see CONFIGURATION.md) |
| **Agents** | 10 in `.claude/agents/` — **4 review/synthesis subagents** (secure-code-reviewer, owasp-llm-reviewer, owasp-agentic-reviewer, knowledge-synthesizer) dispatched in parallel for context isolation + bounded permissions; the **cerberus** security specialist; **2 standing seats** of the research→compose pipeline — **prometheus** (research) and **calliope** (writing, drafts-only); and **3 front-door seats** — **athena** (your vault: find / relate / contradict / gather / remember), **helios** (the day: one synthesised morning brief), **hephaestus** (the tune-up: one prioritised health + hygiene report). Seats are **additive** — every verb they route to still works standalone and stays documented as first-class |
| **Workflows** | 3 multi-agent orchestrations in `.claude/workflows/` — `deep-research` (self-verifying, cited research with a 3-vote verify + re-queue loop) and `devils-advocate` (adversarial pre-mortem for hard-to-reverse decisions: framing gate → hostile lenses + a steelman counterweight → adversarial verify + dissent-quota watch → grounding gate → verdict). Run by the `Workflow` tool; fan out across many subagents with control flow in code, and keep only findings that survive independent refutation. `review` (one-command security-review pipeline: scope → parallel applicable lenses → dedupe → adversarial FP-check per finding → verified severity-ranked list, `/safe-rebuild` surfaced not run). Run by the `Workflow` tool; fan out across many subagents with control flow in code, and keep only findings that survive independent refutation. Read + reason only |
...
- **Test suite is single-shot, not adversarial.** 16 LLM-behaviour scenarios + 35 deterministic checks; no automated adversarial validation yet — **Artemis**, the offensive/adversarial-validation seat (the counterpart to the defensive Cerberus), is on the roadmap, RoE-gated.
...
**Just shipped (v0.29.0):** `/backup-brain` — offline backup and restore for the half of a brain nobody syncs (`~/.claude`: memory files, session summaries, pipeline state), plus a restore offer built into the installer. Recent releases before it: MITRE ATLAS technique tagging with a check that fails invented IDs (v0.28.0), and a guard against untrusted text being laundered into the trusted zone (v0.27.0).
...
**Coming next — a second way in.** A voice interface: hold a key, speak, and it arrives in the live session as typed text. Not a voice-command menu over a few features — the same interface, so anything that can be asked can be asked aloud, with speech-to-text running **locally** so no audio leaves the machine. The reason it was built is accessibility: every other capability here quietly assumes you can sit and type. It is **in test on the author's machine now** and ports to Charon once it runs on Windows, macOS and Linux — the current implementation is Win32, so that is a rewrite of the guard layer rather than a copy. Honest edges up front: it is push-to-talk, so **not hands-free**; it cannot approve anything consequential; and a grounded answer takes about eighty seconds today. See [`ROADMAP.md`](ROADMAP.md) for the full limits and the security control that lets dictation near a terminal at all.
...
| `test-scenarios/` | Pre-release reliability checks — 16 LLM scenarios + 35 automated checks |
