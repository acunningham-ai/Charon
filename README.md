...
| **Hooks** | 16 hooks — load-rules (rule injection), save-on-mention (two-stage Haiku-classified), poisoning-scan (screens untrusted spans of a prompt), poisoning-scan-read (screens captured files at read time, ships in shadow), deny-destructive (with the confirmation-token channel), validate-write-path, validate-memory-frontmatter, voice-anchor-ralph-loop, skill-usage-log, ssh-recovery, notification-toast, check-reauth-flag, check-todo-freshness, validate-interactive-write (interactive write confinement, ships in shadow), phase-gate (high-stakes confirm beat, ships disabled), cerberus/scan-config-edits (config-poisoning scan, ships in shadow) — plus `on-error.py`, invoked by scheduled runners on failure |
...
**Coming next — a second way in.** A voice interface: hold a key, speak, and it arrives in the live session as typed text. Not a voice-command menu over a few features — the same interface, so anything that can be asked can be asked aloud, with speech-to-text running **locally** so no audio leaves the machine. The reason it was built is accessibility: every other capability here quietly assumes you can sit and type. It is **in test on Windows now**, with macOS and Linux as planned updates rather than preconditions. See [`ROADMAP.md`](ROADMAP.md) for the full limits and the security control that lets dictation near a terminal at all.
