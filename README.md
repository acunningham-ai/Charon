...
| **Path-conditioned rules** | 13 path-rules auto-inject doctrine when the path/keyword fires (board-reporting, ai-governance, secure-code, captures, quarterly-report, voice-content, skill-authoring, verdict-vocabulary, versioning, design, backlink-discipline, clean-room-port, thoroughness-over-false-efficiency) | `.claude/rules/*.md` + `scripts/load-rules.py` (UserPromptSubmit hook) |
...
`test-scenarios/` ships with the harness — 16 LLM-behaviour scenarios + 26 automated deterministic checks. The same suite runs before any release and after any material change to rules / hooks / wizard. Pass-rate threshold is published; releases with a failing scenario must document it in known-limitations.
...
| **Path-conditioned rules** | 13 rules — auto-injected on path/keyword match: board-reporting, ai-governance, secure-code, captures, quarterly-report, voice-content, skill-authoring, verdict-vocabulary, versioning, design, backlink-discipline, clean-room-port, thoroughness-over-false-efficiency |
| **Slash commands** | 49 commands across reporting + governance, security review, the research→compose pipeline (`/prometheus`, `/calliope`, `/forum-agenda`), the knowledge-graph build (`/graph-backfill`, `/vault-query`), retrieval + session (`/recall`, `/find-tensions`, `/grill`, `/handoff`), the governed self-* pair (`/harness-doctor`, `/harness-improve`), doc/web utilities (`/ingest`, `/webfetch`, `/docs`, `/skill-eval`), workflow, hygiene, and the 5-command Cerberus suite — full catalogue with fire conditions in [`CAPABILITIES.md`](CAPABILITIES.md) |
