...
`test-scenarios/` ships with the harness — 16 LLM-behaviour scenarios + 27 automated deterministic checks. The same suite runs before any release and after any material change to rules / hooks / wizard. Pass-rate threshold is published; releases with a failing scenario must document it in known-limitations.
...
- **Test suite is single-shot, not adversarial.** 16 LLM-behaviour scenarios + 27 deterministic checks; no automated adversarial validation yet — **Artemis**, the offensive/adversarial-validation seat (the counterpart to the defensive Cerberus), is on the roadmap, RoE-gated.
...
| `test-scenarios/` | Pre-release reliability checks — 16 LLM scenarios + 27 automated checks |
...
| First-run wizard | ✓ |
| Test suite (16 scenarios + 27 checks) | ✓ |
